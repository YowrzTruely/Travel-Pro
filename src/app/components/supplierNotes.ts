...
export function computeAutoNotes(
  ...
  const warningDocs = documents.filter(
    (d) => getDocExpiryStatus(d.expiry) === "warning"
  );
  ...
  const missingDocs = REQUIRED_DOCS.filter(
    (name) => !existingDocNames.has(name)
  );
...
export function computeAutoNotesFromSummary(
  ...
  const otherExpired = summary.insuranceExpired
    ? summary.docsExpired - 1
    : summary.docsExpired;
  if (otherExpired > 0) {
    notes.push({
      id: "docs-expired",
      ...
      level: "critical",
      text:
        otherExpired === 1 ? "מסמך פג תוקף" : `${otherExpired} מסמכים פגי תוקף`,
      icon: "file-warning",
    });
  }
  ...
  if (summary.docsWarning > 0) {
    notes.push({
      ...
      level: "warning",
      text:
        summary.docsWarning === 1
          ? "מסמך עומד לפוג"
          : `${summary.docsWarning} מסמכים עומדים לפוג`,
      icon: "clock",
    });
  }
